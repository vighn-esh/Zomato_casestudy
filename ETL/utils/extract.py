...
def clean_data(df):
    df = df.dropna()  # Drop missing values as part of cleaning
    df.loc[:, 'RATING'] = pd.to_numeric(df['RATING'], errors='coerce')  # Convert 'RATING' to numeric
    return df

# Function to enrich data with latitude and longitude
def enrich_data(df, output_file="enriched_data.csv"):
    # Ensure 'Latitude' and 'Longitude' columns exist before processing
...
def extract(input_path, output_file, enriched_file="restaurant_data_with_lat_long_parallel.csv"):
    logging.info("Starting extraction process...")

    # Check if the enriched file already exists
    if os.path.exists(enriched_file):
        logging.info(f"{enriched_file} exists. Loading data from the file.")
        df = pd.read_csv(enriched_file)
        return df  # Skip loading and enriching, return the existing data

    # Step 1: Load the Data (if enriched file doesn't exist)
    df = load_data(input_path, output_file)
    
    if df is None or df.empty:
        logging.error("No data loaded, extraction process aborted.")
        return None
    ...
    # Step 3: Enrich the Data with Latitude and Longitude
    logging.info("Enriching data with latitude and longitude...")
    enriched_df = enrich_data(cleaned_df, output_file)

    # Save the enriched data to the file for future use
